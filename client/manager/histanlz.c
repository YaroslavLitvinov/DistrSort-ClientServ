...
/*@return two dimensional array, where result[i][j] i part of sorted data, j*/
struct request_data_t**
alloc_range_request_analize_histograms( struct file_records_t *records, const int single_src_items_count, int nodeid,
	...
	do{
		...
		while( range_count < single_src_items_count ){
				 ...
				 range_count + len*histogram_len >= single_src_items_count)
			{
				...
				request_assign_detailed_histogram( records,
						histogram_len, single_src_items_count, nodeid, workers, len, last_histograms_requested );
...
void
request_assign_detailed_histogram( struct file_records_t *records,
		int current_histogram_len, int single_src_items_count, int nodeid,
	...
	for( int i=0; i < array_len; i++ ){
		struct file_record_t* read_dhist_req_r = match_file_record_by_fd( records, i+MANAGER_FD_READ_D_HISTOGRAM_REQ);
		struct file_record_t* write_dhist_req_r = match_file_record_by_fd( records, i+MANAGER_FD_WRITE_D_HISTOGRAM_REQ);
		assert(read_dhist_req_r);
		assert(write_dhist_req_r);
		struct Histogram *detld_histogram = reqrep_detailed_histograms_alloc(
				write_dhist_req_r->fpath, read_dhist_req_r->fpath, nodeid, &request_detailed_histogram[i], last_request );
